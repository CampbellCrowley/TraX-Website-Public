  ...
  (function(Video, undefined) {
    ...
    Video.stopRecording = function(preventDelayedTrigger) {
      ...
      if (!preventDelayedTrigger) {
        if (confirm(
            'The videos have been recorded. Would you like to download ' +
                'them now?\nYou may download then from the options menu ' +
                'indiviually.')) {
          setTimeout(function() {
            const delayed =
                document.getElementsByClassName('DELAYEDDOWNLOADLINK');
    ...
    function releaseCamera() {
      ...
      for (let i = 0; i < localStreams.length; i++) {
        const tracks = localStreams[i].getTracks();
     ...
     * @param {boolean} [delay=false] Delay this download by some period of time
