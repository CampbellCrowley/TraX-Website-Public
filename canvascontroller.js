...
Canvases.Sphere3d = function(radius) {
  ...
  for (let direction = 1; direction >= -1; direction -= 2) {
    for (let beta = pointAngleDelta; beta < Math.PI / 2.0 - pointAngleDelta;
      ...
      let r = Math.cos(beta) * this.radius;
      let Y = Math.sin(beta) * this.radius * direction;
 ...
 *
 * @param {boolean} [makeDown=false] Make the orientation sphere point down to
 * world down, or screen down.
 */
Canvases.renderGyro = function(makeDown) {
  ...
  if (makeDown) {
    for (let i = 0; i < rotatedPoints.length; i++) {
      ...
      let tmp = new TraX.Common.Point3d(rotatedPoints[i]);
  ...
  // Draw all points.
  for (let i = 0; i < rotatedPoints.length; i++) {
    let point = rotatedPoints[i];

    let x = projection(point.x, point.z, gyroWidth / 2.0, 100.0, distance);
    let y = projection(point.y, point.z, gyroHeight / 2.0, 100.0, distance);
...
Canvases.renderAccel = function(externalDevice) {
  ...
  if (!externalDevice) {
    let accelMag = Math.abs(Math.hypot(Ax, Ay, Az));
    let rotMag = Math.abs(
        Math.hypot(
...
Canvases.resetEstimatedVelocity = function(gpsSpeed, gpsHeading) {
  ...
  let newHeading = gpsHeading / 180.0 * Math.PI;
  ...
  let estimatedHeading = Math.atan2(point.y, point.x);
