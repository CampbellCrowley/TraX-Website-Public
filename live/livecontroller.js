...
let polyline = new google.maps.Polyline(polyLineOpts);
 ...
 * @param {boolean} [force=false] Force fetching from server instead of aborting
...
function newData(friendId, chunkId, buffer) {
  if (buffer && buffer.length > 0 && friendId) {
    ...
    if (buffer &&
      ...
      let willUpdateStandings = !friendsData[friendId] ||
      ...
      let splitName = buffer.selectedTrack.split(',');
      let ownerId = splitName[1];
      let trackId = splitName[0];
 ...
 * @param {boolean} [doMinimal=false] Only update row, not the rest of the UIs.
 */
function updateFriendRow(id, doMinimal) {
  ...
  if (friendsData[id]) {
    let isStale = Date.now() - friendsData[id].receivedTime > staleDelay;
...
function updateStandingsList() {
  ...
  for (let i = 0; i < 10 && i < friendStandings.length; i++) {
    let id = friendStandings[i].userId;
...
function updateFriendMap() {
  ...
  Object.keys(friendsData).map(function(key) {
    if (key == 'length') return;
    let label = getText('label', key);
    let pos = {lat: friendsData[key].latitude, lng: friendsData[key].longitude};
...
function purgeData() {
  ...
  Object.keys(friendsData).map(function(key) {
    ...
    if (now - friendsData[key].receivedTime > maxDataAge) {
      let row = document.getElementById('listRow' + key);
...
function updateRealtime(id) {
  ...
  if (friendsData[id].longitude) {
    let gpsLongitude = friendsData[id].longitude + '\u00B0';
  ...
  if (friendsData[id].latitude) {
    let gpsLatitude = friendsData[id].latitude + '\u00B0';
  ...
  if (friendsData[id].heading) {
    let heading = friendsData[id].heading + '\u00B0';
  ...
  if (friendsData[id].gpsTimestamp) {
    let gpsTime = TraX.Common.formatTime(friendsData[id].gpsTimestamp, true);
  ...
  let gpsSpeed = TraX.Units.speedToUnit(friendsData[id].speed) +
      TraX.Units.getLargeSpeedUnit();
  ...
  let deviceType = TraX.getDeviceType(friendsData[id].userAgent);
  ...
  let browser = TraX.getBrowser(friendsData[id].userAgent);
...
function sendNotif(body) {
  if (Notification && Notification.permission == 'granted') {
    notifOpts.body = body;
    let n = new Notification(notifTitle, notifOpts);
