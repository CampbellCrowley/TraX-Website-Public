...
TraX.init = function() {
  ...
  // Register service worker to allow for offline loading, or fasting loading of
  // cached version.
...
function pageDeath() {
  ...
  if ((!isPaused || realtimeViewOpen) && heartbeatCount < 2) {
    ...
    // This is disabled because the user doesn't really care if we missed data,
...
function updateLapTimes() {
  ...
  if (justFinishedRacing && distFinish > previousCoord.distFinish) {
    ...
    // This needs to get the same results as post-analysis since we can save the
