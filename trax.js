...
io.on('connection', function(socket) {
  ...
  function fetchDataLimit() {
    ...
    requestsComplete = function() {
      ...
      if (currentTier >= 0) dataLimit = patreonTiers[currentTier].dataLimit;
      else dataLimit = 100 * 1000 * 1000;  // 100MB
    };
    let numRes = 0;
    const resCount = 2;
  ...
  socket.on('streamsession', function(sessionId, otherId, startByte, endByte) {
    if (streaming) return;
      ...
      fs.stat(sessionFile, function(err, stats) {
        ...
        stream.on('readable', function() {
          ...
          while (null !== (chunk = stream.read())) {
            ...
            bytessent += size;
            socket.emit(
                'streamresponse', size, chunk.toString(), sessionId, bytessent,
                numbytes);
          }
        });
 ...
 * @param {number} otherId A friend Id that must be provided if the filename is
 ...
 *
 * @param {number} attempts Number of different failure recovery options we have
 * remaining to try.
 ...
 * @param {boolean} [isPublic=false] Should this stream be sent to everybody not
 * just friends?
 */
 ...
 * @param {callback} callback Callback containing only the returned object as an
 * argument.
 */
